...
// Revenant Friend.
import me.rina.racc.client.social.RevenantSocialUser; // not implemented yet bc idk how to use Rina's social user yet.

// Revenant Tessallator.
import me.rina.racc.util.render.RevenantTessellator2;

// Revenant Color Util.
import me.rina.racc.util.render.RVColor2;
...
public class RevenantBedAura extends RevenantModule
{

    private RevenantSetting placeESP = newSetting(new String[]{"PlaceESP", "BAPlaceESP", "PlaceESP"}, true);
    private RevenantSetting announceUsage = newSetting(new String[]{"Announce Usage", "BAAnnounce Usage", "Announce Usage"}, true);
    private RevenantSetting range = newSetting(new String[]{"Range", "BARange", "Range"}, 7, 0, 9);
    private RevenantSetting pDelay = newSetting(new String[]{"PlaceDelay", "BAPlaceDelay", "PlaceDelay"}, 15, 0, 20);

    public RevenantBedAura(){
        super("BedAura", "BedAura", "Breaks and places beds", Category.COMBAT);
    }
    ...
    public void onDisable() {
        ...
        if (announceUsage.getBoolean()) {
            RevenantChatUtil.sendClientMessage(TextFormatting.BLUE + "[" + TextFormatting.GOLD + "BedAura" + TextFormatting.BLUE + "]" + ChatFormatting.RED.toString() + " Disabled" + ChatFormatting.RESET.toString() + "!");
        }
    ...
    public void onUpdate() {
        ...
        if(mc.player.dimension == 0) {
            RevenantChatUtil.sendClientMessage("You are in the overworld!");
            this.toggle();
        }
        try {
            findClosestTarget();
        ...
        if(closestTarget == null && mc.player.dimension != 0) {
            if(firstRun) {
                firstRun = false;
                if(announceUsage.getBoolean()) {
                    RevenantChatUtil.sendClientMessage(TextFormatting.BLUE + "[" + TextFormatting.GOLD + "BedAura" + TextFormatting.BLUE + "]" + TextFormatting.WHITE + " enabled, " + TextFormatting.WHITE + "waiting for target.");
                }
            }
        }
        ...
        if (firstRun && closestTarget != null && mc.player.dimension != 0) {
            ...
            if (announceUsage.getBoolean()) {
                RevenantChatUtil.sendClientMessage(TextFormatting.BLUE + "[" + TextFormatting.GOLD + "BedAura" + TextFormatting.BLUE + "]" + TextFormatting.WHITE + " enabled" + TextFormatting.WHITE + ", target: " + ChatFormatting.BLUE.toString() + lastTickTargetName);
            }
        }
        ...
        if(closestTarget != null && lastTickTargetName != null) {
            if (!lastTickTargetName.equals(closestTarget.getName())) {
                lastTickTargetName = closestTarget.getName();
                if (announceUsage.getBoolean()) {
                    RevenantChatUtil.sendClientMessage(TextFormatting.BLUE + "[" + TextFormatting.GOLD + "BedAura" + TextFormatting.BLUE + "]" + TextFormatting.WHITE + " New target: " + ChatFormatting.BLUE.toString() + lastTickTargetName);
                }
            }
        }
        ...
        try {
            ...
            mc.world.loadedTileEntityList.stream()
                    ...
                    .filter(e -> mc.player.getDistance(e.getPos().getX(), e.getPos().getY(), e.getPos().getZ()) <= range.getInteger())
            ...
            if((mc.player.ticksExisted % pDelay.getInteger() == 0)  && closestTarget != null) {
    ...
    public void render(RenderWorldLastEvent event) {
        ...
        if(placeESP.getBoolean()) {try {
            float posx = placeTarget.getX();
            float posy = placeTarget.getY();
            float posz = placeTarget.getZ();
            RevenantTessellator2.prepare("lines");
            RevenantTessellator2.draw_cube_line(posx, posy, posz, RVColor2.GenRainbow(), "all");
            if(rotVar == 90) {
                RevenantTessellator2.draw_cube_line(posx - 1, posy, posz, RVColor2.GenRainbow(), "all");
            }
            if(rotVar == 0) {
                RevenantTessellator2.draw_cube_line(posx, posy, posz + 1, RVColor2.GenRainbow(), "all");
            }
            if(rotVar == -90) {
                RevenantTessellator2.draw_cube_line(posx + 1, posy, posz, RVColor2.GenRainbow(), "all");
            }
